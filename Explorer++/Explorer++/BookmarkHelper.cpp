...
HTREEITEM CBookmarkTreeView::InsertFolderIntoTreeView(HTREEITEM hParent,const CBookmarkFolder &BookmarkFolder,std::size_t Position)
{
	...
	m_mapID.insert(std::pair<UINT,GUID>(m_uIDCounter,BookmarkFolder.GetGUID()));
	++m_uIDCounter;

	m_mapItem.insert(std::pair<GUID,HTREEITEM>(BookmarkFolder.GetGUID(),hItem));
	...
	const GUID &guid,bool bFolder,int iPosition)
{
	...
	m_mapID.insert(std::pair<UINT,GUID>(m_uIDCounter,guid));
	++m_uIDCounter;
